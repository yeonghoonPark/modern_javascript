 ...
 * 이때 식별자와 스코프는 실행 컨텍스으틔 렉시컬 환경으로 관리되며 실행 순서는 실행 컨텍스트의 스택으로 관리한다.
 */
...
// 이때 전역 변수에 값이 할당되고 함수가 호출된다. (즉, `const` 키워드를 사용한 전역 변수인 `a`와 `b`에 값이 할당된다)
// 순차적으로 코드를 실행하는 중 `foo` 함수가 호출되면 순차적으로 실행되던 전역 소스코드 실행은 일시 중단하고 코드 실행 순서를 변경하여 함수 내부로 진입한다.
// 🔑 코드가 일시적으로 중단된다는 의미는 호출 스택(`Call stack`)에 쌓인 상태라는 의미다.
...
// 🔑 함수 소스코드 실행될 때 호출 스택에 쌓이고 실행이 종료되면 호출 스택에서 해당 함수 실행 컨텍스트가 빠져나온다는 것을 의미한다.
// 호출 스택은 `LIFO`(Last In First Out)의 구조를 가지며 스택이 종료되면 전역 소스코드를 실행하는 부분으로 돌아가 순차적으로 코드를 끝까지 실행한다.
